...
export function drawCityAsset(ctx,key,x,y,options){var def=CITY_ASSET_DEFS[key];if(!def)return;options=options||{};var scale=options.scale==null?.28:options.scale;var rotation=options.rotation||0;var alpha=options.alpha==null?1:options.alpha;var anchorX=options.anchorX==null?.5:options.anchorX;var anchorY=options.anchorY==null?.85:options.anchorY;var dw=def.w*scale,dh=def.h*scale;ctx.save();ctx.globalAlpha=alpha;ctx.translate(x,y);ctx.rotate(rotation);if(cityAtlas.complete&&cityAtlas.naturalWidth>0)ctx.drawImage(cityAtlas,def.x,def.y,def.w,def.h,-dw*anchorX,-dh*anchorY,dw,dh);ctx.restore()}
