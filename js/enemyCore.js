...
function chooseCombatTarget(e,player,covers){
  ...
  candidates.forEach(function(t){
    ...
    score+=Math.max(0,900-d)*.035; score+=blocked?-24:30; score+=t.exposed?20:-8;
    if(t===player)score+=18; if(t.role==='flanker')score+=10; if(t.role==='marksman')score+=7;
    score+=Math.random()*22; if(score>bestScore){bestScore=score;best=t}
  });
  return best||player;
}
...
function chooseCover(e,target,covers,enemies,forceNew){
  ...
  var choice=pickTacticalCover(e,target,covers,enemies,{desiredRange:desiredRange(e),maxTravel:1250,minThreat:role==='breach'?180:role==='flanker'?220:300,maxThreat:role==='precision'?1550:role==='marksman'?1400:1250,flankSide:(role==='flanker'||role==='assault')?e.flankSide:0,flankWeight:role==='flanker'?300:role==='assault'?160:80,forceNew:!!forceNew});
...
function shouldReposition(e,target,covers){
  if(!coverStillUseful(e,target,covers,190,e.weapon.role==='precision'?1600:1350))return true;
  if(e.coverCycles<2)return false;
  var chance=e.weapon.role==='flanker' ? .48 : e.weapon.role==='breach' ? .38 : e.weapon.role==='precision' ? .12 : .23;
  if(e.hp<e.maxHp*.35)chance*=.45; return Math.random()<chance;
}
function applySquadHit(target,amount){
  if(!validTarget(target))return;
  target.hp=Math.max(0,target.hp-amount); target.hit=.22; target.timeSinceDamage=0;
  if(target.hp<=0){target.hp=0;target.downed=true;target.downTimer=0;target.exposed=false;target.callout='MAN DOWN';target.calloutTimer=1.7}
}
...
export function updateBandits(enemies,dt,player,covers,spawnProjectile){
  ...
  for(var i=0;i<enemies.length;i++){
    ...
    if(!e.cover&&e.repositionCooldown<=0){
      if(!chooseCover(e,threat,covers,enemies,false)){var dx=threat.x-e.x,dy=threat.y-e.y,d=Math.hypot(dx,dy)||1;e.targetX=threat.x-dx/d*desiredRange(e);e.targetY=threat.y-dy/d*desiredRange(e);e.combatState='seeking';e.exposed=true}
    ...
    if(e.combatState==='seeking'){
      e.exposed=true; if(moveTowardTarget(e,dt,1.08)){if(e.cover)enterCovered(e);else{e.combatState='exposed';e.combatTimer=rand(1,1.5);e.shotsLeft=3+Math.floor(Math.random()*3)}}
    }else if(e.combatState==='covered'){
      e.exposed=false;e.targetX=e.coverAnchorX;e.targetY=e.coverAnchorY;moveTowardTarget(e,dt,1);faceThreat(e,threat);e.combatTimer-=dt;if(e.combatTimer<=0&&dist<e.weapon.range*1.18&&!e.weapon.reloading)enterExposed(e,threat);
    }else if(e.combatState==='exposed'){
      e.exposed=true;if(e.cover){var pp=peekPoint(e,threat,e.cover.type==='wide'?62:48);e.targetX=pp.x;e.targetY=pp.y;moveTowardTarget(e,dt,.88)}faceThreat(e,threat);e.combatTimer-=dt;if(e.combatTimer<=0||e.shotsLeft<=0){if(e.cover)enterTucking(e);else{e.combatState='seeking';e.repositionCooldown=0}}
    }else if(e.combatState==='tucking'){
      e.exposed=true;e.targetX=e.coverAnchorX;e.targetY=e.coverAnchorY;var tucked=moveTowardTarget(e,dt,1.18);e.combatTimer-=dt;if(tucked||e.combatTimer<=0)enterCovered(e);
    }
    dist=Math.hypot(threat.x-e.x,threat.y-e.y);
    var lineBlocked=isLineBlocked(e,threat,covers); var chance=Math.max(8,Math.min(98,getHitChance(e,threat,covers)+e.weapon.accuracy)); e.lastHitChance=chance;
    ...
    if(e.fire<=0&&canFire){
      ...
      if(spawnProjectile){var hitShot=Math.random()*100<chance;if(threat===player){spawnProjectile(e,threat,'enemy',hitShot?e.weapon.damage:0)}else{spawnProjectile(e,threat,'enemy',0);if(hitShot)applySquadHit(threat,e.weapon.damage)}}
    }
  }
}
