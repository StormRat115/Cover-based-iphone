import { isLineBlocked, getHitChance } from "./cover.js?v=20260906-100";
import { weaponCopy } from "./weapons.js?v=20260906-100";
import {
  moveTowardTarget,
  faceThreat,
  coverStillUseful,
  peekPoint,
} from "./combatAI.js?v=20260906-100";
import {
  ENEMY_STATS,
  mitigateDamage,
  combatAccuracy,
  attackDamage,
} from "./combatStats.js?v=20260906-100";
...
import { orderDefense } from "./squadDialog.js?v=20260906-100";
import { assignEnemyCover } from "./enemyCoverAI.js?v=20260906-100";
import { chargerWeapon } from "./chargerEnemy.js?v=20260906-100";
import { tagEnemyStance, seeksCover, applyExposedHold } from "./enemyStance.js?v=20260906-100";
...
function chooseCombatTarget(e, player, covers, enemies) {
  ...
  enemies.forEach(function (o) {
    if (o === e || o.dead || !o.combatTarget || o.combatTarget === player)
      return;
    var n = o.combatTarget.name || "ally";
    allyFocus[n] = (allyFocus[n] || 0) + 1;
  });
  var best = null,
    bestScore = -Infinity;
...
function shouldReposition(e, target, covers) {
  if (
    !coverStillUseful(
      ...
      covers,
      190,
      e.weapon.role === "precision" ? 1600 : 1350,
    )
  )
    return true;
  if (e.coverCycles < 2) return false;
  var chance =
    e.weapon.role === "flanker"
      ? 0.48
      : e.weapon.role === "breach"
        ? 0.38
        : e.weapon.role === "precision"
          ? 0.12
          : 0.23;
  if (e.hp < e.maxHp * 0.35) chance *= 0.45;
...
function applySquadHit(target, amount) {
  ...
  if (target.hp <= 0) {
    ...
    target.downTimer = 0;
    target.exposed = false;
    target.callout = "MAN DOWN";
    target.calloutTimer = 1.7;
  }
}
  ...
  for (var i = 0; i < enemies.length; i++) {
    ...
    if (!e.cover && e.repositionCooldown <= 0) {
      if (!chooseCover(e, threat, covers, enemies, false)) {
        ...
        e.targetX = threat.x - (dx / d) * desiredRange(e);
        e.targetY = threat.y - (dy / d) * desiredRange(e);
        e.combatState = "seeking";
        e.exposed = true;
      }
      e.repositionCooldown = 0.45;
    }
    if (
      e.cover &&
      e.combatState !== "seeking" &&
      e.combatState !== "tucking" &&
      shouldReposition(e, threat, covers) &&
      e.repositionCooldown <= 0
    ) {
      if (chooseCover(e, threat, covers, enemies, true))
        e.repositionCooldown = 1;
    }
    if (
      dist < 240 &&
      e.repositionCooldown <= 0 &&
    ...
    if (e.weapon.ammo <= 0 && !e.weapon.reloading) {
      ...
      AudioBus.playReload({ volume: 0.4 });
    ...
    // Expensive visibility/accuracy work is only needed when a shot is due.
    if (
      e.fire > 0 ||
      e.weapon.reloading ||
      !e.exposed ||
      ...
      if (spawnProjectile) {
        ...
        if (threat === player) {
          spawnProjectile(
            ...
            threat,
            "enemy",
            hitShot ? Math.max(4, raw * 0.65) : 0,
          );
        } else {
          spawnProjectile(e, threat, "enemy", 0);
          if (hitShot) applySquadHit(threat, raw);
        }
      }
    }
  }
