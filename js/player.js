export function createPlayer() {
  const p = {
    ...
    reloading: false,
    reloadTimer: 0,
    weapon: { magazine: 24, ammo: 24, range: 560, fireCooldown: 0.12 },
    keyboardMove: null,
    setDestination(x, y, cover) {
      this.tx = x;
      this.ty = y;
      this.cover = cover;
    ...
    startReload() {
      if (this.reloading || this.weapon.ammo === this.weapon.magazine) return;
      this.reloading = true;
      this.reloadTimer = 1.35;
      this.state = 'reload';
    },
    fireAt(enemy) {
      if (this.reloading || this.weapon.ammo <= 0 || this.weapon.fireCooldown > 0) return false;
      this.weapon.ammo--;
      this.weapon.fireCooldown = 0.12;
      this.state = 'shoot';
      enemy.hp -= 24;
      if (enemy.hp <= 0) {
        enemy.hp = 0;
    ...
    update(dt, covers) {
      ...
      if (this.keyboardMove) {
        ...
        this.state = 'walk';
        if (covers) {
          const near = covers.find(
            (c) => Math.abs(this.x - c.x) < c.w / 2 + 20 && Math.abs(this.y - c.y) < c.h / 2 + 16
          );
          this.cover = near || null;
        }
        return;
      }
...
export function drawPlayer(ctx, p, iso) {
  ...
  ctx.save();
  ctx.translate(sx, sy - Math.sin(p.anim * 10) * 2);
  ctx.fillStyle = '#0008';
  ctx.beginPath();
  ctx.ellipse(0, 3, 27, 10, 0, 0, Math.PI * 2);
  ctx.fill();
  if (p.cover) {
    ctx.strokeStyle = '#8fb7c8';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(0, -8, 30, Math.PI, Math.PI * 2);
    ctx.stroke();
  }
