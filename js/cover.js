...
export function getCoverSlot(c,actor,threat){let side;if(threat){side=threat.y<c.y?'bottom':'top'}else{side=actor.y<c.y?'top':'bottom'}const inset=c.type==='wide'?24:18;const slot=actor&&Number.isFinite(actor.coverSlotIndex)?Math.max(0,Math.min(2,actor.coverSlotIndex)):0;const offsets=[-1,0,1];const usable=Math.max(18,c.w/2-inset);const spread=c.type==='wide'?Math.min(58,usable*.78):Math.min(44,usable*.78);const x=Math.max(c.x-c.w/2+inset,Math.min(c.x+c.w/2-inset,c.x+offsets[slot]*spread));const y=side==='top'?c.y-c.h/2-28:c.y+c.h/2+28;return{x,y,side}}
