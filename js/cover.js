...
export function getCoverSlot(c,actor,threat){let side;if(threat){side=threat.y<c.y?'bottom':'top'}else{side=actor.y<c.y?'top':'bottom'}const inset=c.type==='wide'?26:20;const slot=actor&&Number.isFinite(actor.coverSlotIndex)?actor.coverSlotIndex:0;const offsets=[-1,0,1];const spread=c.type==='wide'?Math.min(30,c.w*.22):Math.min(24,c.w*.2);const x=Math.max(c.x-c.w/2+inset,Math.min(c.x+c.w/2-inset,c.x+offsets[Math.max(0,Math.min(2,slot+1))]*spread));const y=side==='top'?c.y-c.h/2-28:c.y+c.h/2+28;return{x,y,side}}
