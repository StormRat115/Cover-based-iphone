import { createGameLoop } from "./gameLoop.js?v=20260908-135";
import {
  worldToScreen,
  screenToWorld as unproject,
  nearestLivingEnemy,
} from "./geometry.js?v=20260908-135";
import { recoverInCover, shouldRecover } from "./recoveryAI.js?v=20260908-135";
import {
  updateBlood,
  drawBlood,
  resetBlood,
} from "./bloodEffects.js?v=20260908-135";
import { updateSquadHud } from "./squadHud.js?v=20260908-135";
import { updateCombatHud } from "./combatHud.js?v=20260908-135";
import { updatePlayerHud } from "./player.js?v=20260908-135";
import { resetSquadCommands } from "./allyCore2.js?v=20260908-135";
import { preferShootTargets } from "./engaged.js?v=20260908-135";
import "./squadDrawer.js?v=20260908-135";
import { createPlayer, drawPlayer } from "./player.js?v=20260908-135";
import {
  createBandits,
  updateBandits,
  drawBandit,
  drawSniperLasers,
} from "./enemy.js?v=20260908-135";
import { createHostileAt, doorHostileType } from "./enemyCore.js?v=20260908-135";
import { seedDemoVariants } from "./enemyVariants.js?v=20260908-135";
import { createAllies, updateAllies, drawAlly } from "./ally.js?v=20260908-135";
import {
  createMarines,
  updateMarines,
  drawMarine,
} from "./marines.js?v=20260908-135";
import {
  createStreetMission,
  updateStreetMission,
  captureSecondsRemaining,
} from "./streetMission.js?v=20260908-135";
import {
  createSupportVehicle,
  updateSupportVehicle,
  drawSupportVehicle,
} from "./supportVehicle.js?v=20260908-135";
import {
  createCover,
  findCoverForPoint,
  drawCover,
...
import {
  ...
  occupancyPenalty,
  reserveCoverSlot,
} from "./coverSlots.js?v=20260908-135";
import { repathIfSlotContested } from "./combatAI.js?v=20260908-135";
import {
  initKeyboard,
  getKeyboardMove,
  isKeyboardFireHeld,
  clearKeyboard,
} from "./input.js?v=20260908-135";
import { initTactical } from "./tactical.js?v=20260908-135";
...
import {
  ...
  waveFullyCleared,
  pendingHostiles,
} from "./waveSegments.js?v=20260908-135";
import {
  grantKillXp,
  grantWaveXp,
  getTeamProgress,
} from "./teamProgress.js?v=20260908-135";
import {
  updateGrenades,
  drawGrenades,
  trySquadGrenades,
  resetGrenades,
} from "./grenades.js?v=20260908-135";
import {
  applyRunModifiers,
  updateMarineReinforcements,
  resetMarineTimer,
} from "./runModifiers.js?v=20260908-135";
import {
  drawWartornAtmosphere,
  drawWartornDressing,
  drawWartornStreetSurface,
} from "./wartornCity.js?v=20260908-135";
import {
  createFacadeDoorDirector,
  resetFacadeDoors,
  updateFacadeDoors,
  drawFacadeDoorBursts,
} from "./facadeDoors.js?v=20260908-135";
import {
  updateFireteams,
  assignFireteams,
} from "./fireteams.js?v=20260908-135";
import {
  updateCombatVfx,
  drawCombatVfx,
  resetCombatVfx,
  notifyShotImpact,
} from "./combatVfx.js?v=20260908-135";
import {
  tickStreetBreath,
  applyStreetBreathHold,
  triggerBreachBeat,
  isStreetBreathing,
} from "./streetBeat.js?v=20260908-135";
import {
  updateSquadDialog,
  drawDialogBubbles,
  resetSquadDialog,
} from "./squadDialog.js?v=20260908-135";
import {
  unstickOverlappingUnits,
  resetUnitUnstick,
} from "./unitCollision.js?v=20260908-135";
import {
  camModeLabel,
  cameraLookAt,
  easeCameraToward,
} from "./frontLineCam.js?v=20260908-135";
import {
  spraySuppression,
...
import {
  ...
  spawnKillAmmo,
  spawnWaveAmmo,
} from "./ammoEconomy.js?v=20260908-135";
import {
  damageCover,
  tickCoverVisuals,
} from "./destructibleCover.js?v=20260908-135";
import {
...
} from "./streetObjectives.js?v=20260908-135";
...
window.__doorDirector = doorDirector;
...
function createWaveEnemies() {
  ...
  try {
    if (typeof location !== "undefined" && /demo-door/.test(String(location.hash || "")))
      doorDirector.timer = 0.45;
  } catch (_err) {}
  seedDemoVariants(roster, createHostileAt);
  window.__doorDirector = doorDirector;
  return roster;
}
function nearestEnemy() {
  return nearestLivingEnemy(player, preferShootTargets(enemies));
}
...
function chooseAutoCombatEnemy() {
  var best = null,
    bestScore = -Infinity,
    pool = preferShootTargets(activeCombatEnemies());
...
function attemptFire() {
  ...
  if (player.lastAttackMelee) {
    player.lastAttackMelee = false;
    if (hit) {
      hitMarker = 0.12;
      damagePops.push({
        x: e.x,
        y: e.y,
        value: Math.round(
          e.lastDamageTaken || (player.melee && player.melee.damage) || 8,
        ),
        t: 0,
      });
...
function chooseAutoPosition(e, strategicGoal) {
  ...
  covers.forEach(function (c) {
    ...
    var slot = reserveCoverSlot(c, player, e, occupants);
    if (!slot) return;
    var protectedSpot = e
...
function updateAutoPlayer(dt) {
  ...
  var d = distance(player, e),
    ...
    objectivePush =
      ...
      (strategicGoal.source === "street" ||
        (strategicGoal.source === "fort" &&
          streetTasks &&
          streetTasks.resumeForward &&
          !streetTasks.current)) &&
  ...
  var slotTeam = [player]
    ...
    .concat(enemies || []);
  if (
    repathIfSlotContested(player, e, covers, slotTeam, {
      advance: !!strategicGoal,
      advancePoint: strategicGoal,
      minThreat: 50,
      maxThreat: 2400,
      maxTravel: 1250,
    })
  ) {
    if (player.cover && Number.isFinite(player.targetX)) {
      player.setDestination(player.targetX, player.targetY, player.cover);
    } else if (Number.isFinite(player.targetX)) {
      player.setDestination(player.targetX, player.targetY, null);
    }
  }
  autoMoveTimer -= dt;
...
function update(dt) {
  ...
  var beforeEnemies = enemies.length;
  var streetResult = updateStreetObjectives(streetTasks, dt, {
    ...
    covers: covers,
    enemies: enemies,
    createHostile: function (x, y, i) {
      return createHostileAt(x, y, doorHostileType(wave, i), {
        index: 30 + i,
        fromDoor: true,
        spawnTimer: 0.06 + i * 0.1,
        doorApproachX: x + (x > 0 ? -90 : 90),
        doorApproachY: y + 40,
      });
    },
  });
  ...
  if (covers.length !== beforeCovers) {
    ...
    rebuildLayers();
  } else if (enemies.length !== beforeEnemies) {
    rebuildLayers();
  }
  ...
  if (breathTick && breathTick.finished) {
    ...
    if (autoPlay && !player.dead && !player.downed) {
      ...
      if (resumeGoal) {
        if (!chooseAutoPosition(resumeEnemy, resumeGoal))
          stepAutoPlayerToward(resumeGoal, 180);
      } else if (resumeEnemy) {
        if (!chooseAutoPosition(resumeEnemy, null))
...
function drawMissionUI() {
  ...
  ctx.fillText(
    objectiveTitle(streetTasks, mission),
    x + 14,
    y + 41,
  );
  ctx.fillStyle = "#8fb7c8";
