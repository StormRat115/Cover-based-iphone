...
let W=0,H=0,dpr=1,last=0,gameOver=false,won=false,target=null,kills=0,hitMarker=0,damagePops=[],fireHeld=false;
...
fireButton.addEventListener('pointerdown',e=>{e.preventDefault();fireHeld=true;fireButton.classList.add('active');attemptFire()});addEventListener('pointerup',()=>{fireHeld=false;fireButton.classList.remove('active')});addEventListener('pointercancel',()=>{fireHeld=false;fireButton.classList.remove('active')});
...
function finish(win){won=win;gameOver=!win;fireHeld=false;messageTitle.textContent=win?'AREA CLEAR':'MISSION FAILED';messageText.textContent=win?'All hostiles eliminated.':'The soldier was killed.';message.classList.remove('hidden')}
function reset(){player.reset();enemies=createBandits();projectiles.length=0;damagePops.length=0;hitMarker=0;target=null;kills=0;fireHeld=false;gameOver=false;won=false;message.classList.add('hidden')}
function update(dt){if(gameOver||won)return;const km=getKeyboardMove();if(km)player.setKeyboardMove(km);else if(player.keyboardMove)player.setKeyboardMove(null);updateBandits(enemies,dt,player,covers);if(player.hp<=0&&!player.dead)player.triggerDeath();player.update(dt);if(fireHeld)attemptFire();updateProjectiles(dt);updateFeedback(dt);if(target?.dead)target=null;if(player.dead&&player.deathTimer>=player.deathDuration){finish(false);return}if(enemies.every(e=>e.dead&&e.deathTimer>=e.deathDuration))finish(true)}
