...
const buildings = [
  { x: -420, y: -280, img: 'ruinA', scale: 2.4 },
  { x: 380, y: -300, img: 'ruinB', scale: 2.2 },
  { x: -450, y: 220, img: 'ruinB', scale: 2.0 },
  { x: 420, y: 180, img: 'ruinA', scale: 2.1 },
  { x: -80, y: -360, img: 'ruinA', scale: 1.8 },
  { x: 100, y: 340, img: 'ruinB', scale: 1.7 },
];
function resize() {
...
function checkWaveClear() {
  ...
  if (wave === 1 && !spawning) {
    ...
    waveAnnounce = 2.2;
    if (waveBanner) {
      waveBanner.textContent = 'WAVE 2';
      waveBanner.classList.remove('hidden');
    }
    setTimeout(() => {
      wave = 2;
      enemies = createBandits(2);
      spawning = false;
      target = null;
...
function finish(win) {
  ...
  messageText.textContent = win
    ? `All hostiles eliminated. Kills: ${kills}. Dylan ${dylan.dead ? 'KIA' : 'survived'}.`
...
function drawAsphalt() {
  ...
  for (let x = -920; x <= 920; x += step) {
    for (let y = -720; y <= 720; y += step) {
      const [cx, cy] = iso(x, y);
      const hw = world.scaleX * (step * 0.52);
      const hh = world.scaleY * (step * 0.52);
      ctx.save();
      ctx.beginPath();
      ctx.moveTo(cx, cy - hh);
      ctx.lineTo(cx + hw, cy);
      ...
      if (tile) {
        const tw = hw * 2.2;
        const th = hh * 2.2;
        const ox = ((x + y) * 0.13) % 40;
        const oy = ((x - y) * 0.09) % 40;
        ctx.globalAlpha = 0.92;
        ctx.drawImage(tile, cx - tw / 2 + ox * 0.2, cy - th / 2 + oy * 0.2, tw, th);
        ctx.globalAlpha = 1;
      } else {
        ctx.fillStyle = (x + y) % (step * 2) === 0 ? '#303a3f' : '#2c353a';
        ctx.fill();
      }
...
function drawBuildings() {
  ...
  for (const b of sorted) {
    const img = b.img === 'ruinA' ? images.ruinA : images.ruinB;
    if (!img) continue;
    ...
    ctx.drawImage(img, sx - w / 2, sy - h * 0.88, w, h);
    ctx.restore();
  }
}
function depthKey(o) {
...
function drawWorld() {
  drawAsphalt();
  drawBuildings();
  covers.forEach((c) => drawCover(ctx, c, iso));
...
function update(dt) {
  ...
  if (dylan.hp <= 0 && !dylan.dead) dylan.dead = true;
  updateBandits(enemies, dt, player, covers, [dylan]);
  if (target?.dead) target = null;
  if (!spawning && enemies.every((e) => e.dead)) checkWaveClear();
}
function draw() {
  drawWorld();
  const sprites = [];
  for (const e of enemies) {
    if (!e.dead) sprites.push({ z: depthKey(e), kind: 'enemy', ref: e });
  }
  if (!dylan.dead) sprites.push({ z: depthKey(dylan), kind: 'dylan', ref: dylan });
  sprites.push({ z: depthKey(player), kind: 'player', ref: player });
  sprites.sort((a, b) => a.z - b.z);
  for (const s of sprites) {
    if (s.kind === 'enemy') drawBandit(ctx, s.ref, iso, target === s.ref);
    else if (s.kind === 'dylan') drawDylan(ctx, s.ref, iso);
    else drawPlayer(ctx, s.ref, iso);
  }
  const alive = enemies.filter((e) => !e.dead).length;
  const coverTag = player.cover ? ' * IN COVER' : '';
  const lockTag = target && !target.dead ? ' * TARGET LOCKED' : '';
  status.innerHTML =
    `<span class="hp">HP ${Math.max(0, Math.ceil(player.hp))}</span>` +
    ` * <span class="ammo">AMMO ${player.weapon.ammo}/${player.weapon.magazine}</span>` +
  ...
  hint.textContent = player.reloading
    ? 'RELOADING...'
    : 'Joystick / WASD move * Tap target * FIRE * R reload';
