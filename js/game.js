...
export function nearestEnemy(){return enemies.filter(e=>!e.dead).sort((a,b)=>distance(player,b)-distance(player,a))[0]||null}

function screenToWorld(sx,sy){const a=(sx-W/2)/world.scaleX;const b=(sy-(H/2+world.offsetY))/world.scaleY;return {x:(a+b)/2,y:(b-a)/2}}
function setTarget(e){target=e;player.aimTarget=e}
function clearTargetIfDead(){if(target?.dead)target=null}
...
function attemptFire(){
 ...
 if(isLineBlocked(player,e,covers))return;
 const result=player.fireAt(e);
 if(result){kills += e.dead ? 1 : 0;if(kills===enemies.length)finish(true)}
}
...
canvas.addEventListener('pointerdown',e=>{
 ...
 for(const enemy of enemies){if(enemy.dead)continue;const [x,y]=iso(enemy.x,enemy.y);if(Math.hypot(sx-x,sy-(y-28))<38){setTarget(enemy);return}}
 const p=screenToWorld(sx,sy);const cover=findCoverForPoint(p.x,p.y,covers);
 if(cover)updatePlayer(p.x,p.y,cover);else updatePlayer(p.x,p.y,null)
});

function updatePlayer(x,y,cover){player.setDestination(x,y,cover)}
...
function update(dt){
 ...
 updateBandits(enemies,dt,player,covers);
 clearTargetIfDead();
 if(enemies.every(e=>e.dead))finish(true);
}
