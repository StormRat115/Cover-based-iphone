...
function reset(){player.reset();enemies=createBandits();projectiles.length=0;target=null;kills=0;gameOver=false;won=false;message.classList.add('hidden')}
function update(dt){if(gameOver||won)return;const km=getKeyboardMove();if(km)player.setKeyboardMove(km);player.update(dt);if(player.hp<=0){finish(false);return}updateBandits(enemies,dt,player,covers);updateProjectiles(dt);if(target?.dead)target=null;if(enemies.every(e=>e.dead))finish(true)}
...
function draw(){drawWorld();drawProjectiles();enemies.filter(e=>!e.dead).sort((a,b)=>(a.x+a.y)-(b.x+b.y)).forEach(e=>drawBandit(ctx,e,iso,target===e));drawPlayer(ctx,player,iso);status.textContent=`HP ${Math.max(0,Math.ceil(player.hp))} • ${kills}/${enemies.length} BANDITS • ${player.weapon.ammo}/${player.weapon.magazine}${player.cover?' • IN COVER':''}${target&&!target.dead?' • TARGET LOCKED':''}`;hint.textContent=player.reloading?'RELOADING…':'Tap cover to take position • WASD move • SPACE/FIRE shoot • R reload';reloadButton.classList.toggle('hidden',player.weapon.ammo===player.weapon.magazine&&!player.reloading)}
