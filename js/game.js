...
function draw(){drawWorld();drawProjectiles();enemies.filter(e=>!e.dead||e.deathTimer<e.deathDuration).sort((a,b)=>(a.x+a.y)-(b.x+b.y)).forEach(e=>drawBandit(ctx,e,iso,target===e));drawPlayer(ctx,player,iso);status.textContent=`HP ${Math.max(0,Math.ceil(player.hp))} • ${kills}/${enemies.length} BANDITS • ${player.weapon.ammo}/${player.weapon.magazine}${player.cover?' • IN COVER':''}${target&&!target.dead?' • TARGET LOCKED':''}`;hint.textContent=player.dead?'SOLDIER DOWN':player.reloading?'RELOADING…':'Tap cover to take position • WASD move • SPACE/FIRE shoot • R reload';reloadButton.classList.toggle('hidden',player.weapon.ammo===player.weapon.magazine&&!player.reloading)}
