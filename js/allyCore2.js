...
export function createAllies(){
  ...
  var allies=SQUAD.map(function(s,i){
    ...
    return {name:s.name,role:s.role,weapon:w,x:pos[0],y:pos[1],hp:s.hp,maxHp:s.hp,lastHp:s.hp,fire:.4+i*.4,t:0,dead:false,permadead:false,downed:false,downTimer:0,downDuration:11,reviveTimer:0,reviveDuration:2.5,deathTimer:0,deathDuration:.8,muzzle:0,hit:0,targetX:pos[0],targetY:pos[1],moveTimer:.2+i*.3,cover:null,coverSlotIndex:i,speed:s.speed,facingX:i===1?-1:1,facingY:0,scale:1,timeSinceDamage:99,regenDelay:3,regenRate:s.regen,callout:'',calloutTimer:0,lastCalloutHealth:s.hp,reloadTimer:0,reloading:false,flankSide:i===1?-1:1};
  });
...
function choosePosition(a,player,covers,allies,enemies,target,mode){
  var available=covers.filter(function(c){
    return Math.hypot(target.x-c.x,target.y-c.y)<1050&&!enemies.some(function(e){return !e.dead&&!e.downed&&e.cover===c});
  });
  if(!available.length){
    a.cover=null;
    a.targetX=player.x+(a.role==='flanker'?a.flankSide*150:(Math.random()-.5)*120);
    a.targetY=player.y+90+Math.random()*120;
    return;
  }
  var desired=a.role==='marksman'?650:a.role==='flanker'?450:mode==='ASSAULT'?330:430;
  available.sort(function(u,v){return Math.abs(Math.hypot(target.x-u.x,target.y-u.y)-desired)-Math.abs(Math.hypot(target.x-v.x,target.y-v.y)-desired)});
  var c=available[Math.min(a.role==='flanker'?1:0,available.length-1)];
  a.cover=c;
  var users=allies.filter(function(o){return o!==a&&!o.dead&&!o.downed&&o.cover===c});
  a.coverSlotIndex=users.length%3;
  var slot=getCoverSlot(c,a,target);
  a.targetX=slot.x+(a.role==='flanker'?a.flankSide*22:0);
  a.targetY=slot.y;
}
...
function separateFromSquad(a,allies){
  ...
  allies.forEach(function(o){
    ...
    if(d<120&&d>0){sx+=dx/d*(120-d);sy+=dy/d*(120-d);n++}
  });
...
function tryRevive(a,allies,player,dt){
  ...
  a.targetX=a.x;a.targetY=a.y;a.cover=null;a.reviveTimer+=dt;
...
export function updateAllies(allies,dt,player,covers,enemies,spawnProjectile,mode){
  ...
  for(var i=0;i<allies.length;i++){
    ...
    a.t+=dt;a.fire-=dt;a.muzzle=Math.max(0,a.muzzle-dt);a.hit=Math.max(0,a.hit-dt);a.calloutTimer=Math.max(0,a.calloutTimer-dt);a.timeSinceDamage+=dt;
    if(a.hp<a.lastHp){a.timeSinceDamage=0;if(a.hp<a.maxHp*.35&&a.calloutTimer<=0)say(a,"I'M INJURED",1.6)}
    a.lastHp=a.hp;
    if(a.hp<=0&&!a.downed){a.downed=true;a.downTimer=0;a.hp=0;say(a,"I'M DOWNED",1.7)}
    if(a.downed){a.downTimer+=dt;if(a.downTimer>=a.downDuration){a.permadead=true;a.dead=true;a.downed=false;a.deathTimer=0}continue}
    if(a.hp<a.maxHp&&a.timeSinceDamage>=a.regenDelay)a.hp=Math.min(a.maxHp,a.hp+a.regenRate*dt);
    if(a.reloading){a.reloadTimer-=dt;if(a.reloadTimer<=0){a.reloading=false;a.weapon.ammo=a.weapon.magazine}}
    if(tryRevive(a,allies,player,dt))continue;

    a.moveTimer-=dt;
    var target=null,dist=Infinity,pick;
    ...
    if(!target){
      a.targetX=player.x+(a.role==='flanker'?a.flankSide*150:(a.x<player.x?-70:70));
      a.targetY=player.y+90+(a.role==='marksman'?-60:60);
      a.cover=null;
    }else{
      var blocked=isLineBlocked(a,target,covers);
      if(a.moveTimer<=0){
        a.moveTimer=a.role==='flanker'?.32:a.role==='marksman'?.7:.5+Math.random()*.45;
        if(mode==='HOLD'&&!a.cover)choosePosition(a,player,covers,allies,enemies,target,'HOLD');
        else if(a.role==='flanker'&&(blocked||dist<390||!a.cover))choosePosition(a,player,covers,allies,enemies,target,mode);
        else if(a.role==='marksman'&&(blocked||dist<480||!a.cover))choosePosition(a,player,covers,allies,enemies,target,mode);
        else if(mode!=='HOLD'&&(blocked||dist<300||!a.cover))choosePosition(a,player,covers,allies,enemies,target,mode);
      }
      if(mode==='FOLLOW'&&dist>720){a.cover=null;a.targetX=player.x+(a.role==='flanker'?a.flankSide*210:(a.x<player.x?-100:100));a.targetY=player.y+(a.role==='marksman'?-90:100)}
      if(mode==='HOLD'&&a.cover&&dist<720){a.targetX=a.x;a.targetY=a.y}
      var sep=separateFromSquad(a,allies);
      if(sep&&mode!=='HOLD'){a.targetX+=sep.x*.55;a.targetY+=sep.y*.55}
      var dx=a.targetX-a.x,dy=a.targetY-a.y,d=Math.hypot(dx,dy);
      if(d>7){a.facingX=dx/d;a.facingY=dy/d;var step=Math.min(d,a.speed*dt);a.x+=dx/d*step;a.y+=dy/d*step}else{a.targetX=a.x;a.targetY=a.y}
      ...
      if(a.weapon.ammo<=0&&!a.reloading){a.reloading=true;a.reloadTimer=a.weapon.reload;say(a,'RELOADING',1.1)}
      if(a.fire<=0&&dist<a.weapon.range&&(!lineBlocked||a.cover)&&!a.reloading){
        a.fire=a.weapon.cooldown*(a.role==='flanker'?.9:1.05)+Math.random()*.22;
        a.facingX=(target.x-a.x)/(dist||1);a.facingY=(target.y-a.y)/(dist||1);a.muzzle=.1;
        if(spawnProjectile&&Math.random()*100<chance)spawnProjectile(a,target,'ally',a.weapon.damage);
      }
    }
  }
}
